...
{
    __unsafe_unretained id _observee; // the trampoline is stored via associated object on
    NSString *_keyPath;
    AAPLBlockObserver _block;
    volatile atomic_bool _cancellationPredicate;
...
- (AAPLObserverTrampoline *)initObservingObject:(id)obj keyPath:(NSString *)keyPath options:(NSKeyValueObservingOptions)options block:(AAPLBlockObserver)block
{
    ...
    _options = options;
    _observee = obj;
    atomic_init(&_cancellationPredicate, false);
    return self;
}
...
- (void)startObserving
{
    ...
    // clear the bit so dispatch_async isn't called unnecessarily in cancelObservation
...
- (void)cancelObservation
{
    ...
    if  (atomic_compare_exchange_strong(&_cancellationPredicate, &expected, true)) {
